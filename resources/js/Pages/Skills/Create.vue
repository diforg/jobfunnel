<template>
    <AppLayout title="Nova Habilidade">
        <div class="max-w-md">
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                <form @submit.prevent="submit" class="space-y-4">
                    <div>
                        <InputLabel>Nome *</InputLabel>
                        <TextInput v-model="form.name" type="text" required placeholder="Ex: Laravel, Docker, PostgreSQL..." />
                        <InputError :message="form.errors.name" />
                    </div>
                    <div>
                        <InputLabel>Categoria</InputLabel>
                        <TextInput v-model="form.category" type="text" placeholder="Ex: Backend, Frontend, DevOps..." />
                    ...
                    <div class="flex items-center justify-end gap-3 pt-3 border-t border-gray-100">
                        ...
                        <PrimaryButton :disabled="form.processing">
                            {{ form.processing ? 'Salvando...' : 'Criar Habilidade' }}
                        </PrimaryButton>
                    </div>
                </form>
            </div>
